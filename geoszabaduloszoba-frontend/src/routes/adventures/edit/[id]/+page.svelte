<script lang="ts">
	...
	const adventureId = page.params.id;
	...
	onMount(async () => {
		if (!auth.token) {
			errorMessage = "Bejelentkezés szükséges!";
			loading = false;
			return;
		}
		...
		try {
			...
			if (res.ok) {
				...
				difficulty = diffMap[data.difficulty] ?? 1;
				...
				if (data.stations && data.stations.length > 0) {
					stations = data.stations.map((s: any) => ({
						...
						content: {
							riddle: s.content?.riddle || "",
							explanation: s.content?.explanation || "",
							hints: s.content?.hints?.length === 3 ? s.content.hints : ["", "", ""]
						}
					}));
				} else {
					addStation();
				...
				goto('/adventures?error=unauthorized');
			} else {
				errorMessage = "Nem sikerült betölteni a kaland adatait.";
			}
		} catch (err) {
			console.error(err);
			errorMessage = "Hálózati hiba a betöltéskor!";
		} finally {
			loading = false;
		}
	});
	...
	async function saveDraft() {
		...
		try {
			...
			if (response.ok) {
				...
				alert("Nem sikerült frissíteni a piszkozatot.");
			}
		} catch (err) {
			console.error(err);
...
<main class="min-h-screen bg-[#F5F2EA] px-6 pt-16 pb-32">
	{#if loading}
		<div class="flex items-center justify-center min-h-[50vh]">
			<div class="w-10 h-10 border-4 border-[#2F5D50] border-t-transparent rounded-full animate-spin"></div>
		</div>
	{:else}
...
{#if showLeaveModal}
	<div class="fixed inset-0 z-[3000] bg-black/60 backdrop-blur-md p-6 flex items-center justify-center">
		<div class="bg-[#F5F2EA] w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl border-2 border-[#8D7462] flex flex-col gap-4 text-center">
			...
			<p class="text-xs font-bold text-[#8D7462]">Szeretnéd elmenteni a módosításokat piszkozatként, mielőtt kilépsz?</p>
